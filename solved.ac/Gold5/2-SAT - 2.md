...
[백준 문제 링크](https://www.acmicpc.net/problem/11278)
...
function solution() {
  ...
  const validCase = getValidCase(n, group);
  printResult(validCase);
}
...
function getValidCase(n, group) {
  const validCase = Array(n);
  for (let num = 1; num <= n; num += 1) {
    ...
    if (group[numTrue] === group[numFalse]) {
      ...
      validCase[num - 1] = group[numTrue] < group[numFalse];
    }
  }
...
function printResult(validCase) {
  if (!validCase) {
    console.log("0");
  } else {
    const validCaseString = validCase.map(Number).join(" ");
    console.log(`1\n${validCaseString}`);
  }
}
