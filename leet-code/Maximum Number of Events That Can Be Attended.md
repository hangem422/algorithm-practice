...
[LeetCode 문제 링크](https://leetcode.com/problems/maximum-number-of-events-that-can-be-attended)

```typescript
function maxEvents(events: number[][]): number {
  const eventCount = events.length;
  const sortedEvent = [...events].sort((event1, event2) => event1[0] - event2[0]);
  const heap = new Heap<number[]>((event1, event2) => event1[1] - event2[1]);

  let index = 0;
  let day = 1;
  let count = 0;

  while (index < eventCount || heap.size > 0) {
    if (heap.size === 0) {
      const firstEvent = sortedEvent[index];
      day = Math.max(firstEvent[0], day);
    }

    for (; index < eventCount && sortedEvent[index][0] <= day; index += 1) {
      heap.add(sortedEvent[index]);
    }

    let hasValidEvent = false;
    while (heap.size > 0) {
      const event = heap.poll() as number[];
      if (event[1] >= day) {
        hasValidEvent = true;
        break;
      }
